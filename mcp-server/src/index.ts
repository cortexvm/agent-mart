...
server.tool(
  "get_manual",
  "Get the step-by-step guide on how to use Agent Mart. Start here if you're new.",
  {},
  async () => {
...
server.tool(
  "browse_products",
  "Browse products available on Agent Mart. Filter by category (books/gadgets), subcategory, or search term.",
  {
    category: z
      .enum(["books", "gadgets"])
      .optional()
  ...
  async ({ category, subcategory, search }) => {
    ...
    const products = data.data.map(
      (p: { id: string; name: string; price: number; category: string; subcategory: string; inStock: boolean; description: string }) =>
        `- **${p.name}** (ID: ${p.id}) - $${p.price.toFixed(2)} [${p.category}/${p.subcategory}] ${p.inStock ? "In Stock" : "OUT OF STOCK"}\n  ${p.description}`
    );
...
server.tool(
  ...
  "Get full details of a specific product by its ID.",
  {
    product_id: z
      .string()
      .describe(
        "The product ID (e.g. 'clean-code', 'laptop-stand', 'dune')"
      ),
  },
  async ({ product_id }) => {
    ...
    if (!data.success) {
      return {
        content: [{ type: "text" as const, text: `Product not found: ${data.error}` }],
        isError: true,
      };
    }
...
server.tool(
  "place_order",
  "Place a Cash on Delivery (COD) order on Agent Mart. You need product IDs, quantities, and customer delivery info.",
  {
    items: z
      .array(
        z.object({
  ...
  async ({
    ...
    if (!data.success) {
      return {
        content: [{ type: "text" as const, text: `Order failed: ${data.error}` }],
        isError: true,
      };
    }
...
server.tool(
  "track_order",
  "Check the status of an existing order by its order ID.",
  {
    order_id: z
      .string()
  ...
  async ({ order_id }) => {
    ...
    if (!data.success) {
      return {
        content: [{ type: "text" as const, text: `Order not found: ${data.error}` }],
        isError: true,
      };
    }
...
server.tool(
  "dashboard",
  "View marketplace dashboard - total orders, revenue, orders by agent, recent orders.",
  {},
  async () => {
    ...
    if (!data.success) {
      return {
        content: [{ type: "text" as const, text: "Failed to load dashboard" }],
        isError: true,
      };
    }
    ...
    const recent = d.recentOrders
      .map(
        (o: { id: string; customer: { name: string }; totalAmount: number; status: string; agentId?: string }) =>
          `  ${o.id} | ${o.customer.name} | $${o.totalAmount.toFixed(2)} | ${o.status} | ${o.agentId || "unknown"}`
      )
      .join("\n");
